...
def tokenize_messages_finetune2(examples, tokenizer, max_length, answer_start_str:str,answer_end_str:str):
    text = examples['text']
    messages_label=[]
    ...
    if len(input_ids)!=len(labels):
        raise ValueError('input_ids和labels长度不一致，input_ids长度：{}，labels长度：{}'.format(len(input_ids),len(labels)))
...
def load_json_datasets(json_dir, tokenizer):
    ...
    for path in tqdm(path_list, desc='加载数据集', total=len(path_list)):
        ...
        # 由messages形成完整prompt，记为text。
            ...
            dataset = dataset.map(lambda x: {"text":tokenizer.apply_chat_template(x['messages'], add_generation_prompt=False, tokenize=False)},
                                  batched=False, num_proc=16)
        else:
...
def main(base_model_path,output_dir,datasedt_dir):
    ...
    with training_args.main_process_first(desc="grouping texts together"):
        ...
        lm_dataset = raw_dataset.map(
            lambda x: tokenize_messages_finetune2(x,
                                                 tokenizer=tokenizer,
    ...
    lora_config = LoraConfig(
        r=16, lora_alpha=64, target_modules=["q_proj", "k_proj", "v_proj","o_proj","gate_proj","up_proj","down_proj"],
